<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <form class="flex flex-col gap-5" action="{{ route('employees.update', $employee->id) }}" method="POST">
				@csrf
				@method('PUT')
				...
				<div class="">
					...
					<select id="company"
							...
							@foreach($companies as $company)
								<option value="{{ $company->id }}" {{ ($employee->company->id == $company->id) ? 'selected' : '' }}>
									{{ $company->name }}
								</option>
							@endforeach
					</select>
				...
				<label class="block">
					...
					<input name="name" type="text" required placeholder="Employee Name" value="{{ old('name') ?: $employee->name }}" class="peer mt-1 block w-full px-3 py-2 bg-slate-300 focus:bg-white border border-slate-300 rounded-md text-sm shadow-sm placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none">
					@error('name')
						<p class="mt-2 text-pink-600 text-sm">
							{{ $message }}
						</p>
				...
				<label class="block">
					...
					<input name="email" type="email" required placeholder="abc@abc.com" value="{{ old('email') ?: $employee->email }}" class="peer mt-1 block w-full px-3 py-2 bg-slate-300 focus:bg-white border border-slate-300 rounded-md text-sm shadow-sm placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none">
					@error('email')
						<p class="mt-2 text-pink-600 text-sm">
							{{ $message }}
						</p>
				...
				<label class="block">
					...
					<input name="phone" type="text" required placeholder="+62 812-3456-7890" value="{{ old('phone') ?: $employee->phone }}" class="peer mt-1 block w-full px-3 py-2 bg-slate-300 focus:bg-white border border-slate-300 rounded-md text-sm shadow-sm placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none">
					@error('phone')
						<p class="mt-2 text-pink-600 text-sm">
							{{ $message }}
						</p>
